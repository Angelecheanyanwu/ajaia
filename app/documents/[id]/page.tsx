...
export default function DocumentPage() {
    ...
    const handleImportToDraft = async (
        event: React.ChangeEvent<HTMLInputElement>,
    ) => {
        const file = event.target.files?.[0];
        if (!file) return;
        ...
        const appendText = async (text: string) => {
            ...
            const newNodes = [
                { type: "horizontalRule" },
                {
                    ...
                    attrs: { level: 3 },
                    content: [
                        { type: "text", text: `Imported from ${file.name}` },
                    ],
                },
                ...lines,
            ];
                        ...
                        {attachments.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {attachments.map((file) => (
                                    <div
                                        key={file.id}
                                        className="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl group transition-all hover:bg-blue-50/50"
                                    >
